...
/**
 * Centralized permission management for Meshora app
...
class PermissionManager(private val context: Context) {
    ...
    companion object {
        ...
        private const val PREFS_NAME = "Meshora_permissions"
    ...
    fun getCategorizedPermissions(): List<PermissionCategory> {
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required to discover Meshora users via Bluetooth",
                permissions = bluetoothPermissions,
                isGranted = bluetoothPermissions.all { isPermissionGranted(it) },
                systemDescription = "Allow Meshora to connect to nearby devices"
            )
        )
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required by Android to discover nearby Meshora users via Bluetooth",
                permissions = locationPermissions,
                isGranted = locationPermissions.all { isPermissionGranted(it) },
                systemDescription = "Meshora needs this to scan for nearby devices"
            )
        )
        ...
        if (shouldRequireWifiAwarePermission()) {
            ...
            categories.add(
                PermissionCategory(
                    ...
                    description = "Enable Wi‑Fi Aware to discover and connect to nearby Meshora users over Wi‑Fi.",
                    permissions = wifiAwarePermissions,
                    isGranted = wifiAwarePermissions.all { isPermissionGranted(it) },
                    systemDescription = "Allow Meshora to discover nearby Wi‑Fi devices"
                )
            )
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            categories.add(
                PermissionCategory(
                    ...
                    systemDescription = "Allow Meshora to send you notifications"
                )
            )
        }
        ...
        if (isBatteryOptimizationSupported()) {
            categories.add(
                PermissionCategory(
                    ...
                    description = "Disable battery optimization to ensure Meshora runs reliably in the background and maintains mesh network connections",
                    permissions = listOf("BATTERY_OPTIMIZATION"), // Custom identifier
                    isGranted = isBatteryOptimizationDisabled(),
                    systemDescription = "Allow Meshora to run without battery restrictions"
                )
            )
        }
